...
export function getCharacterCardFieldsLazy({ chid = undefined } = {}) {
    ...
    const resolvers = {
        ...
        activeCharContent: () => {
            if (groupCardsLazy) return groupCardsLazy.activeCharContent || '';
            if (!character) return '';
            return baseChatReplace(character.activeCharContent?.trim());
        },
        combinedCharacters: () => {
            if (groupCardsLazy) return groupCardsLazy.combinedCharacters || '';
            if (!character) return '';
            return baseChatReplace(character.combinedCharacters?.trim());
        },
    };
